...
from authorship_tool.types import OneDimStr, Tag, TaggedToken, TokenStr, TwoDimStr
...
class SentenceCalculator:
    ...
    @staticmethod
    def word_variation(sent: OneDimStr) -> float:
        """
    ...
    @staticmethod
    def average_token_length(sent: OneDimStr) -> float:
        """
        文章中の単語の平均文字列長を計算する
    ...
    @staticmethod
    def non_alphabetic_characters_frequency(sent: OneDimStr) -> float:
        """
        文章内で出現する記号の割合を計算する
    ...
    @staticmethod
    def uncommon_word_frequency(sent: OneDimStr) -> float:
        """
        特徴的な単語の割合を計算する
    ...
    @staticmethod
    def pos_frequencies(sent: OneDimStr) -> dict[Tag, float]:
        """
        文章中の各品詞の割合を計算する
    ...
    @staticmethod
    def sentence_length(sent: OneDimStr) -> int:
        """文章中に出現するトークン数を計算する"""
        return len(sent)

    @staticmethod
    def count_individual_tokens(sent: OneDimStr) -> int:
        """文章中に出現する単語の種類数を計算する"""
        return len(set(sent))

    @staticmethod
    def count_character(sent: OneDimStr, character: str) -> int:
    ...
    @staticmethod
    def count_non_alphabetic_characters(sent: OneDimStr) -> int:
    ...
    @staticmethod
    def count_uncommon_words(sent: OneDimStr) -> int:
...
class UnivKansasFeatures:
    ...
    @staticmethod
    def v1_sentences_per_paragraph(para: TwoDimStr) -> int:
        """段落内の文数を計算する"""
        return len(para)

    @staticmethod
    def v2_words_per_paragraph(para: TwoDimStr) -> int:
        """段落内で出現する単語の合計を計算する"""
        sent: OneDimStr = dim_reshaper.reduce_dim(para)
        return SentenceCalculator.sentence_length(sent)

    @staticmethod
    def _char_present(para: TwoDimStr, char: str) -> bool:
        """段落内に指定した文字が存在するかどうかを判定する"""
        return any(char in word for sent in para for word in sent)

    @staticmethod
    def v3_close_parenthesis_present(para: TwoDimStr) -> bool:
        """段落内に括弧閉じが存在するかどうかを判定する"""
        return UnivKansasFeatures._char_present(para, ")")

    @staticmethod
    def v4_dash_present(para: TwoDimStr) -> bool:
        """段落内にダッシュが存在するかどうかを判定する"""
        return UnivKansasFeatures._char_present(para, "-")

    @staticmethod
    def _semi_colon_present(para: TwoDimStr) -> bool:
        """段落内にセミコロンが存在するかどうかを判定する"""
        return UnivKansasFeatures._char_present(para, ";")

    @staticmethod
    def _colon_present(para: TwoDimStr) -> bool:
        """段落内にコロンが存在するかどうかを判定する"""
        return UnivKansasFeatures._char_present(para, ":")

    @staticmethod
    def v5_semi_colon_or_colon_present(para: TwoDimStr) -> bool:
    ...
    @staticmethod
    def v6_question_mark_present(para: TwoDimStr) -> bool:
        """段落内に疑問符が存在するかどうかを判定する"""
        return UnivKansasFeatures._char_present(para, "?")

    @staticmethod
    def v7_apostrophe_present(para: TwoDimStr) -> bool:
        """段落内にアポストロフィが存在するかどうかを判定する"""
        return UnivKansasFeatures._char_present(para, "'")

    @staticmethod
    def v8_standard_deviation_of_sentence_length(para: TwoDimStr) -> float:
        """段落内の文長の標準偏差を計算する"""
    ...
    @staticmethod
    def v9_length_difference_for_consecutive_sentences(para: TwoDimStr) -> float:
    ...
    @staticmethod
    def v10_sentence_with_lt_11_words(para: TwoDimStr) -> bool:
        """段落内に単語数が11未満の文が存在するかどうかを判定する"""
        return any(len(sentence) < 11 for sentence in para)

    @staticmethod
    def v11_sentence_with_gt_34_words(para: TwoDimStr) -> bool:
        """段落内に単語数が34より多い文が存在するかどうかを判定する"""
        return any(len(sentence) > 34 for sentence in para)

    @staticmethod
    def _contains_word(para: TwoDimStr, word: str) -> bool:
    ...
    @staticmethod
    def v17_contains_others_or_researchers(para: TwoDimStr) -> bool:
    ...
    @staticmethod
    def v18_contains_numbers(para: TwoDimStr) -> bool:
    ...
    @staticmethod
    def _count_char(para: TwoDimStr, target_char: str) -> int:
        """段落内に指定した文字の合計を計算する"""
        return sum(
            target_char == char
            for sentence in para
    ...
    @staticmethod
    def v19_contains_2_times_more_capitals_than_period(para: TwoDimStr) -> bool:
    ...
    @staticmethod
    def v20_contains_et(para: TwoDimStr) -> bool:
